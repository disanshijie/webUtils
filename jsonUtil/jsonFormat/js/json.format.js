...
function JsonFormater(opt) {
    this.options = $.extend({
        ...
        singleTab: " ",
        quoteKeys: true,
        imgCollapsed: jsurlbase + "images/Collapsed.gif",
        imgExpanded: jsurlbase + "images/Expanded.gif",
        isCollapsible: true
    }, opt || {});
...
JsonFormater.prototype = {
    ...
    doFormat: function (json) {
        ...
        try {
            ...
            $(this.options.dom).html("<pre class='CodeContainer'>" + html + "</pre>").show();
            if ($('#CollapsibleView').prop('checked') && !$(".validateMsg").hasClass("validateErr")) {
                $("#expandcollapse").show();
            } else {
                $("#expandcollapse").hide();
            }
    ...
    ProcessObject: function (obj, indent, addComma, isArray, isPropertyContent) {
        ...
        if (this.isArray(obj)) {
            if (obj.length == 0) {
                ...
                clpsHtml = this.options.isCollapsible ? "<span><img class='imgToggle' data-status='1' src='" + this.options.imgExpanded + "'/></span><span class='collapsible'>" : "";
